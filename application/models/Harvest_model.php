...
class Harvest_model extends CI_Model {
    ...
    public function get_harvest_list(){
        $this->db->select('upcoming_harvest.*,user.first_name,user.last_name,crops.crop_name');
        $this->db->from('upcoming_harvest');
        $this->db->join('user','user.id = upcoming_harvest.farmer_id');
        $this->db->join('crops','crops.id = upcoming_harvest.crop_id');
